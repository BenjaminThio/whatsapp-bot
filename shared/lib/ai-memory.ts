...
export async function saveHistory(chatId: string, history: HistoryTurn[]): Promise<HistoryTurn[]> {
  ...
  Measured in BYTES, not string length. `.length` counts UTF-16 code units, so
  a conversation in Chinese or full of emoji was undercounted by up to 3x and
  could sail past the limit it was meant to enforce.

  Halving repeatedly rather than once: a single huge turn can still leave the
  row oversized after one cut, and the loop is bounded by the array emptying.
  */
  while (trimmed.length > 1 && Buffer.byteLength(JSON.stringify(trimmed), "utf-8") > MAX_SERIALISED_BYTES) {
