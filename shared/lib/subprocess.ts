...
/*
The workspace root - one level above shared/. Both bots live beside it, so a
single .venv there serves both instead of each carrying its own copy of the
same packages (that was 542 MB + 172 MB of duplicated gTTS, rembg and Pillow).
*/
...
/** Candidate interpreters for a given root, in preference order. */
function venvCandidates(root: string): string[] {
    return isWindows
        ? [path.join(root, ".venv", "Scripts", "python.exe")]
        : [path.join(root, ".venv", "bin", "python"), path.join(root, ".venv", "bin", "python3")];
}

/**
 * Resolve the Python interpreter to use.
 *
 * Looks for a venv at the workspace root first, then at the caller's own root,
 * then falls back to the system interpreter. Termux has no venv at all and
 * uses the system python against the .py sources directly, which is why the
 * fallback matters as much as the venv does.
 *
 * `PYTHON_BIN` overrides everything, for hosts where the interpreter lives
 * somewhere unusual.
 */
export function resolvePython(projectRoot: string): string {
    ...
    for (const candidate of [...venvCandidates(WORKSPACE_ROOT), ...venvCandidates(projectRoot)]) {
        if (existsSync(candidate)) return candidate;
    }

    // No venv (the normal Termux case). Prefer python3 - some distros ship
    // only that, and on those a bare `python` is either absent or Python 2.
