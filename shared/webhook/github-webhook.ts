...
/*
Firestore is optional. Every entry point resolves it first and throws a message
the user can act on, rather than crashing on an undefined handle.
*/
async function requireDb() {
  const db = await getFirestoreDb();
  if (!db) {
    throw new Error(
      "GitHub webhooks need Firebase credentials. Set FIREBASE_SERVICE_ACCOUNT " +
      "or place serviceAccountKey.json in the bot folder."
    );
  }
  return db;
}
...
export interface WebhookConfig {
  ...
  targetJid:  string;        // chat to notify (WhatsApp jid, or Telegram chat id)
  transport?: "whatsapp" | "telegram";   // which bot delivers it
...
export async function createWebhook(
  ...
  await (await requireDb()).collection(COLLECTION).doc(token).set(config);
  return config;
}

export async function getWebhook(token: string): Promise<WebhookConfig | null> {
  const snap = await (await requireDb()).collection(COLLECTION).doc(token).get();
...
export async function listWebhooks(ownerJid: string): Promise<WebhookConfig[]> {
  const snap = await (await requireDb()).collection(COLLECTION)
...
export async function deleteWebhook(token: string, ownerJid: string): Promise<boolean> {
  const ref = (await requireDb()).collection(COLLECTION).doc(token);
