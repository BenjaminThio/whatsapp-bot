...
/**
 * A stored student credential.
 *
 * This used to be declared inside the WhatsApp bot's !test command, which meant the
 * data layer imported a type from a command module and the Telegram bot could
 * not reuse either. It belongs here.
 */
export interface Creds {
    /** UTAR student id, 7 digits. */
    id: string;
    /** UTAR email, @1utar.my. */
    email: string;
    /** Mask the id in reports and rankings. */
    hidden: boolean;
    /** Set when someone registered this account on another person's behalf. */
    ownerId?: string;
}
