...
export async function findIsolatedSessions(
  ...
  /*
  Fetched in parallel, a few at a time.

  Each slotsForDoc() is a real HTTP request to the portal, and this used to run
  them one after another: with ten accounts and a one-second response that is
  ten seconds of the command doing nothing. A small pool gets almost all of the
  speedup without opening ten sockets at once on a phone, or looking like a
  scraper to the far end.
  */
  const PORTAL_CONCURRENCY = Number(process.env["PORTAL_CONCURRENCY"] ?? 4);

  const fetched = await mapLimitSettled(
    others,
    PORTAL_CONCURRENCY,
    async ([docId]) => await slotsForDoc(docId)
  );

  const otherKeys = new Set<string>();
  let compared = 0;

  for (const slots of fetched) {
